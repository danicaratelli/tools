function fcst = fcstBVAR(model,Par,FIT)
...
for j = start_eval-hor:end_eval-1
    ...
    %% get the predictions from BVAR (the original data are augmented by
    %% the prediction as additional rows)
    Xpr = bvarLitt(x,p,pi,mu,hor,iRW);
    ...
    for h = 1:hor                                       %% loop across horizon
        ...
        rw(j+h,:,h)  = mean(x(h+1:end,:)-x(1:end-h,:)); %% benchmark forecast
    end;
