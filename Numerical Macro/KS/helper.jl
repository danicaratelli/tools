...
function simulate_HH(NN,Nsimul,astart,simulZ,dist_employment,cpol,Kgrid,agrid)
    ...
    for i=2:Nsimul
        ...
        #low assets and low capital
        C_alow_Klow = dist_employment[:,i-1].*cpol[2*(simulZ[i-1]-1)+2,locas,locK] .+ (1 .- dist_employment[:,i-1]).*cpol[2*(simulZ[i-1]-1)+1,locas,locK];
        A_alow_Klow_e = (1 + (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*r(Kgrid[locK],Zs[simulZ[i-1]]))*agrid[locas] .+ (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*w(Kgrid[locK],Zs[simulZ[i-1]]) .- C_alow_Klow;
        A_alow_Klow_u = (1 + (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*r(Kgrid[locK],Zs[simulZ[i-1]]))*agrid[locas] .+ ζ*(1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*w(Kgrid[locK],Zs[simulZ[i-1]]) .- C_alow_Klow;
        A_alow_Klow = dist_employment[:,i-1].*A_alow_Klow_u .+ abs.(1 .- dist_employment[:,i-1]).*A_alow_Klow_e;    #asset distribution when high

        #high assets and low capital
        C_ahigh_Klow = dist_employment[:,i-1].*cpol[2*(simulZ[i-1]-1)+2,locas.+1,locK] .+ (1 .- dist_employment[:,i-1]).*cpol[2*(simulZ[i-1]-1)+1,locas.+1,locK];
        A_ahigh_Klow_e = (1 + (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*r(Kgrid[locK],Zs[simulZ[i-1]]))*agrid[locas.+1] .+ (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*w(Kgrid[locK],Zs[simulZ[i-1]]) .- C_ahigh_Klow;
        A_ahigh_Klow_u = (1 + (1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*r(Kgrid[locK],Zs[simulZ[i-1]]))*agrid[locas.+1] .+ ζ*(1-τ(Kgrid[locK],Zs[simulZ[i-1]]))*w(Kgrid[locK],Zs[simulZ[i-1]]) .- C_ahigh_Klow;
        A_ahigh_Klow = dist_employment[:,i-1].*A_ahigh_Klow_u .+ abs.(1 .- dist_employment[:,i-1]).*A_ahigh_Klow_e;

        #low assets and high capital
        C_alow_Khigh = dist_employment[:,i-1].*cpol[2*(simulZ[i-1]-1)+2,locas,locK+1] .+ (1 .- dist_employment[:,i-1]).*cpol[2*(simulZ[i-1]-1)+1,locas,locK+1];
        A_alow_Khigh_e = (1 + (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*r(Kgrid[locK+1],Zs[simulZ[i-1]]))*agrid[locas] .+ (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*w(Kgrid[locK+1],Zs[simulZ[i-1]]) .- C_alow_Khigh;
        A_alow_Khigh_u = (1 + (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*r(Kgrid[locK+1],Zs[simulZ[i-1]]))*agrid[locas] .+ ζ*(1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*w(Kgrid[locK+1],Zs[simulZ[i-1]]) .- C_alow_Khigh;
        A_alow_Khigh = dist_employment[:,i-1].*A_alow_Khigh_u .+ abs.(1 .- dist_employment[:,i-1]).*A_alow_Khigh_e;

        #high assets and high capital
        C_ahigh_Khigh = dist_employment[:,i-1].*cpol[2*(simulZ[i-1]-1)+2,locas.+ 1,locK+1] .+ (1 .- dist_employment[:,i-1]).*cpol[2*(simulZ[i-1]-1)+1,locas.+ 1,locK+1];
        A_ahigh_Khigh_e = (1 + (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*r(Kgrid[locK+1],Zs[simulZ[i-1]]))*agrid[locas.+1] .+ (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*w(Kgrid[locK+1],Zs[simulZ[i-1]]) .- C_ahigh_Khigh;
        A_ahigh_Khigh_u = (1 + (1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*r(Kgrid[locK+1],Zs[simulZ[i-1]]))*agrid[locas.+1] .+ ζ*(1-τ(Kgrid[locK+1],Zs[simulZ[i-1]]))*w(Kgrid[locK+1],Zs[simulZ[i-1]]) .- C_ahigh_Khigh;
        A_ahigh_Khigh = dist_employment[:,i-1].*A_ahigh_Khigh_u .+ abs.(1 .- dist_employment[:,i-1]).*A_ahigh_Khigh_e;
