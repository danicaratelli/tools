...
function NewtonRaphson_secant(f, x0, x1, Xs, tol = 1e-8, maxiter = 10000)
    ...
    it = 0
    @assert ((x0 >= Xs[1]) &&
             (x0 <= Xs[end]) && (x1 >= Xs[1]) && (x1 <= Xs[end])) "Starting value is not in function support Xs"
...
function Broyden_secant(f, J, xstart, Xs, tol = 1e-8, maxiter = 10000)
    Jnew = J;
    ...
    #making sure starting x is in support
    for n = 1:ndim
        @assert ((xold[n] >= Xs[n, 1]) && (xold[n] <= Xs[n, end])) "Starting value is not in function support Xs"
    end
